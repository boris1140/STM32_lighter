...
/* USER CODE BEGIN PV */
uint32_t ticks = 0;
uint8_t is_lighten = 0;
uint8_t light_color = COLOR_RED;
...
int main(void) {
    ...
    */
    ...
    while (1) {
        uint32_t arr_value = __HAL_TIM_GET_AUTORELOAD(&htim5) + 1;
        ...
        */
        /*
        if (HAL_GPIO_ReadPin(KEY_GPIO_Port, KEY_Pin) == GPIO_PIN_RESET && is_lighten == 0) {
            HAL_Delay(10);
            if (HAL_GPIO_ReadPin(KEY_GPIO_Port,KEY_Pin) == GPIO_PIN_RESET) {
                is_lighten = 1;
                ticks = HAL_GetTick();
        ...
        if (HAL_GPIO_ReadPin(KEY_GPIO_Port, KEY_Pin) == GPIO_PIN_SET && is_lighten == 1) {
            HAL_Delay(10);
